    ...
    switch (pObjIndex->item_type)
    {
        ...
        case ITEM_FOUNTAIN:
            fprintf(fp, "%d %d '%s' %d %d\n",
                pObjIndex->value[0],
                pObjIndex->value[1],
                liquid_table[pObjIndex->value[2]].liq_name,
...
void save_class(int num)
{
    ...
    fprintf(fp, "AttrSecond  %s\n", flag_string(stat_flags, class_table[num]->attr_second));
    ...
    for (race = 0; race < MAX_PC_RACE; race++)
    {
        ...
        // If the multiplier is greater than 0, save it, if it's 0 or less than 0 then
        // write out the default 100.
        if (pc_race_table[race].class_mult[num] > 0)
        {
            fprintf(fp, "Mult %d %d\n", race, pc_race_table[race].class_mult[num]);
        }
        else
        {
            fprintf(fp, "Mult %d 100\n", race);
        }
    }
