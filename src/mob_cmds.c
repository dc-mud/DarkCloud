...
/*
 * Lets the mobile set a rot timer on an item.  It can also put a
 * rot timer on a worn item (it will remove it)
 *
 * Syntax: mob rot [item] [ticks until rot]
 */
void do_mprot(CHAR_DATA * ch, char *argument)
{
    char arg1[MAX_INPUT_LENGTH];
    char arg2[MAX_INPUT_LENGTH];
    OBJ_DATA *obj;
    ...
    if (!IS_NPC(ch))
    {
        bug("mprot: mprot does not work on PC's.", 0);
        return;
    }

    // Make sure we have arguments passed
    if (IS_NULLSTR(argument))
    {
        return;
    }

    // Put each argument into it's own
    argument = one_argument(argument, arg1);
    argument = one_argument(argument, arg2);

    // Make sure they are both populated with an argument
    if (IS_NULLSTR(arg1) || IS_NULLSTR(arg2))
    {
        bug("mprot: Empty argument 1 or 2 from mob vnum %d", IS_NPC(ch) ? ch->pIndexData->vnum : 0);
        return;
    }

    // Make sure the second argument is a number, which is the number of ticks until
    // the found item in the mobs inventory has until it rots (item has to be in their
    // inventory)
    if (!is_number(arg2))
    {
        bug("mprot: Rot timer argument on mob %d was not a number.", IS_NPC(ch) ? ch->pIndexData->vnum : 0);
        return;
    }

    // Turn the timer into a integer.
    timer_value = atoi(arg2);

    // Loop through all of the objects in the mobs inventory, look for one that matches
    // the keyword provided.  This will match for ALL objects are founc.
    for (obj = ch->carrying; obj != NULL; obj = obj_next)
    {
        obj_next = obj->next_content;
        ...
        {
            // If the mob is wearing it, have them remove it.
            if (obj->wear_loc != WEAR_NONE)
            {
                unequip_char(ch, obj);
            }
