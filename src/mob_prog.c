    ...
    const void *arg1, const void *arg2, CHAR_DATA * rch)
{
    ...
    {
        case CHK_AFFECTED:
            // First, lookup to see if the flag exists, if not make it 0 instead of NO_FLAG
            // which is 99.
            id = flag_lookup(buf, affect_flags);
            ...
            // Check if either a bit affect or a skill affect of the name exist.
            return (lval_char != NULL
                && (IS_SET(lval_char->affected_by, id)
                    || is_affected(lval_char, skill_lookup(buf))));
        case CHK_ACT:
            return (lval_char != NULL
