...
/*
 * Returns the position in the flag table of a given item.
 */
int flag_position_lookup(const char *name, const struct flag_type *flag_table)
{
    int pos;

    for (pos = 0; flag_table[pos].name != NULL; pos++)
    {
        if (LOWER(name[0]) == LOWER(flag_table[pos].name[0])
            && !str_prefix(name, flag_table[pos].name))
            return pos;
    }
...
int liq_lookup(const char *name)
{
    ...
    {
        if (LOWER(name[0]) == LOWER(liquid_table[liq].liq_name[0])
            && !str_prefix(name, liquid_table[liq].liq_name))
            return liq;
    }
